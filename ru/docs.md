...
```
В объекте this.node['nodename'].reactivity описаны все реактивные функции используемые в node. Если удалить этот объект, то пропадет вся реактивность для данного узла.

## deleteReactive
```js
this.deleteReactive(this.node['nodename'].reactivity, 'keypath')
```
С помощью этой функции можно отменять реактивность для конкретных прокси объектов. Для этого первым аргументом нужно передать reactivity узла, а вторым имя прокси в формате 'keypath'.
