...
Или можете подключить проект глобально, используя сервис: [jsdelivr](https://www.jsdelivr.com/)

Файлы для подключения находятся на github.com в директории /dist с постфиксом `.global`
...
Работа фреймворка начинается с вызова функции createWidget или createApp.
...
- `options`:
	...
	- `stores` объекты stores.
	- `router` объект роутера.
	- `directives` глобальные директивы.
	- `components` глобальные компоненты, будут доступны во всех компонентах через `this.component`.
	- `common` шина данных, все свойства этого объекта будут доступны в store и во всех компонентах через `this`. Подходит для интеграции сторонних библиотек и модулей. Крайне не рекомендуется использовать шину для передачи данных между компонентами.
...
Получить доступ к элементу узла в компоненте можно через `this.node['имя узла']`.
...
- `create` Выполняется при обработке узла компонента, и работает с двумя аргументами: 
  - `node` - это объект узла, к которому применяется директива.
  - `options` - это параметры директивы, применяемой к данному узлу.

- `update` Выполняется для каждого параметра директивы. Работает с тремя аргументами:
  - `node` и `options` Аналогично, как в функции `create`.
  - `key` Ключ параметра директивы. Если параметр является реактивной функцией, то update будет вызываться при каждом реактивном изменении этого параметра.

- `destroy` Выполняется когда компонент демонтируется.
...
```
`sections` позволяет интегрировать из родительского компонента в дочерний другие компоненты, в заранее выбранные теги. В template дочернего компонента необходимо задать теги с атрибутами section, значениями этих атрибутов будут именами секций. В родительском компоненте в component properties нужно описать объект sections. Свойства этого объекта будут соответствующие имена секций. Значениями этих свойств могут быть component properties: src, params, proxies, methods. Если component properties отсутствуют, то компонент можно будет интегрировать динамически. Коммуникация с такими компонентами доступна только из родителя.

> Аналогично как и функции нативных свойств узлов, описанных выше, функции params, proxies в качестве аргумента принимают объект узла, со всеми дополнительными свойствами. Только в свойстве **nodepath** к строке добавляется имя секции. Не работает для компонентов со свойством iterate.
...
this.node['имя узла'].section['имя слота'].unmount()
...
## deleteReactive
```js
console.log(this.node['nodename'].reactivity)
this.deleteReactive(this.node['nodename'].reactivity, 'keypath')
```
С помощью этой функции можно отменять реактивность для конкретных прокси объектов. Для этого первым аргументом нужно передать reactivity объект узла, а вторым имя прокси в формате 'keypath'.
В объекте this.node['nodename'].reactivity описаны все реактивные функции используемые в node. Если удалить этот объект, то пропадет вся реактивность для данного узла. Доступно в компонентах, через `this`.

## execute
```js
textContent: () => this.execute(() => this.proxy.foo)
```
Эта функция позволяет получить данные прокси, не включая его в реактивность для функции в которой он используется. Доступно в компонентах, через `this`.
...
```
Функция replicate делает глубокое клонирование объекта с помощью сериализации. Работает только с типами данных которые поддерживает JSON, для клонирования объектов с другими типами можно использовать нативную javascript функцию [structuredClone()](https://developer.mozilla.org/docs/Web/API/structuredClone).
...
```
Функция используется для более короткой записи props. Применяется как для `methods`, так и для `params` и `proxies`.
Функция принимает дла аргумент:
- Массив с перечислением ключей принимаемых в `props`
- Объект с общими для всех ключей опциями.

Возвращает объект, который можно добавить к основному через оператор [spread](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Operators/Spread_syntax).
...
```
Функция предназначена для временной задержки выполнения кода.

Принимает в качестве аргумента время в миллисекундах.

Возвращает promise со свойством `process`, значение которого является `true` пока promise выполняется.
...
```
Функция используется для управления частотой вызова функций во избежание избыточной обработки повторных вызовов.
Функция принимает два аргумента:
- Функция, для которой нужно управлять частотой вызова
- Время в миллисекундах, задержка перед вызовом функции.
...
## queue
```js
import { queue } from 'lesta'
...
const endpointQueue = queue()
const fetchFromEndpoint = async (endpoint, callback) => {
...
const getData = (endpoint) => {
  endpointQueue.add(() => fetchFromEndpoint(endpoint, handleResult))
}
```
Функция обработки очереди. Вы можете добавлять сколько угодно функций в очередь и они будут выполняться одна за другой в порядке, в котором были добавлены.

Функция `queue()` возвращает объект с тремя методами: 
- `add` Используется для добавления функций в очередь.
- `isEmpty` Возвращает `true`, если очередь пуста.
- `size` Возвращает количество функций в очереди.
...
```
Если вам нужно сделать объект доступным только для чтения вы можете использовать нативную javascript функцию [Object.freeze()](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object/freeze). Для объектов имеющих вложенность используйте deepFreeze.
