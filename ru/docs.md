...
```
Чтобы использовать Lesta, загрузите последнюю версию или установите ее с помощью [npm](https://www.npmjs.org/) (`npm i lesta`).

Lesta использует классические модели декларативного и компонентного программирования. Объект компонента представляет собой набор опций.
...
```
[ Пример ]: https://github.com/lestejs/examples/blob/main/CounterWidget/index.js

Это легковесная функция, с помощью которой можно создать только один компонент (Виджет). С этой функцией доступны только базовые возможности фреймворка: Реактивность, Хуки жизненного цикла и тд. Все возможности доступные для виджетов далее будут помечены “*”.

Функция принимает два параметра:

**component options** - опции компонента, описаны ниже и помечены “*”
**root** - DOM узел для встраивания widget

createWidget возвращает объект с функцией destroy. widget.destroy() уничтожит widget.
...
app.mount(component)
```
[пример]: https://github.com/lestejs/examples/blob/main/ToDoList/index.js

С помощью этой функции можно создать полноценное приложение. функция принимает объект со следующими опциями:

**root** - DOM узел для монтирования
**stores** - объекты stores
**router** - объект роутера
**directives** - глобальные директивы (см ниже)
...
createApp объект с двумя функциями mount и unmount

В функцию mount необходимо передать объект компонента. Если нужно размонтировать компонент используется unmount. Можно вызвать с await.
app.unmount()
...
Node является DOM узлом, поэтому в нем доступны все соответствующие методы и свойства (такие textContent или innerHTML, onclick, style тд.). Функция nodes должна возвращать объект. Ключами объекта являются имена узлов. Имена узлов должны соответствовать именам классов заданных в тегах html. Для изменения селектора используется selectors (см. ниже).

Чтобы устанавливать значения для нативных свойства узла, достаточно в параметрах узла указать свойство в качестве ключа объекта, в качестве значения указать либо значение, либо функцию, которая будет возвращать значение.
...
Используя деструктуризацию, можно из объекта узла получить нужные свойства. Например: 
...
> Свойства params не доступны для расширения. Нельзя добавить новый param после их создания. При этом существующие изменяются без ограничений.
...
**keypath** соответствует ключам вложенных свойств объекта proxy разделенными знаком “.”. Если proxy не имеет вложенности, то имя имя функции будет соответствовать имени proxy. Для proxy имеющих свойства, можно отслеживать изменение конкретных свойств. Например, если в proxy объекте **user: { age: 25 }** необходимо отследить изменение свойства **age**, то имя функции должно быть ‘user.age’(). Так как в javascript массив также является объектом, можно отслеживать изменение его элементов или длины: **array.0, array.0.name, array.length**, где array имя массива.

handlers особенно удобны, когда прокси меняется из нескольких мест и нужно выполнять одно и тоже действие при его изменении.
...
Чтобы монтировать компонент в узел необходимо задать component properties в свойстве узла component:

Параметр src ожидает объект компонента, который можно предварительно импортировать или определить в текущем файле.

Передавать данные в дочерний компонент можно через параметры: params, proxies, methods.

**params** Данные не требующие отслеживания. Чтобы в params передать ссылку на node необходимо использовать стрелочную функцию. popup: () => this.node.popup

**proxies** proxy объекты, которые будут обновляться на стороне дочернего компонента, работает только от родителя к потомку. Чтобы переданные прокси объекты были реактивными, в свойство необходимо передать функцию, в которой присутствует proxy объект. Такая функция может возвращать вычисляемые значения. 

Для глубинной синхронизации proxy, необходимо, чтобы имя свойства начиналось со символа “_”. Функция не должны иметь вычислений, она должны возвращать только proxy.
...
type (string) - проверка на соответствие типу.
	...
	store (string) - имя стора из которого необходимо принять prop.
	readonly (boolean) - делает param неизменяемым.
...
proxy переданный в компонент обязательно должен быть принят в props.
Валидация значений происходит не только при создании компонента, но и в runtime (не работает для proxy имя которых которых начинается с “_” (одинарное подчеркивание), так как изменение может касаться любого уровня вложенности несчем соотнести параметры валидации).
...
> Все полученый props становятся частью компонента и доступны как this.param, this.proxy, this.method. 
...
Эта функции удаляет компонент из узла. this.node[‘имя узла’].unmount()

> Если в узле присутствует метод unmount, значит в данный узел уже был вмонтирован компонент.

## refresh
```js
```
С помощью этой функции можно обновить компонент в узле. Функци предварительно выполняет unmount, затем компонент заново монтируется с исходными параметрами.
...
async add(notify) {
      this.proxy.notifications.unshift(notify)
      await animate(null, null, { delay: this.param.time })
      this.proxy.notifications.pop()
    }
```
```js
if (incomplete) {
        this.param.delayFilter = animate(null,null,{ delay: 2000 })
...
```
## uid*
С помощью этой функции можно генерировать случайный uid в формате xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx. Это формат UUID v4, который используется в стандарте RFC 4122.
