...
Для чтобы процесс в погружения возможностей фреймворка был более приятным и эффективным. В документации используется код и полноценных демонстрационных проектов. Поэтому в каждом разделе будет указана ссылка на файл в котором этот код используется. Также для каждой фичи прикреплены ссылки на файлы сценариев тестирования.
...
```
Функция `createWidget` - легковесная функция, с помощью которой можно создать только один компонент (виджет).
...
Чтобы монтировать компонент в узел необходимо задать component properties в свойстве узла componen, через опции можно передавать данные в дочерний компонент:
