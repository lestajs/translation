...
Встроенные плагины:
this.isMobileDevice: Определяет мобильное устройство.
this.isBrowser: Определяет браузер.
...
## section mounting
```js
this.node.popup.sections.content.mount({
	src: form,
	params: {
	  card: task,
	},
...
```
Функции mount позволяет динамически интегрировать компонент в секцию. Для этого в функцию необходимо передать два параметра.

this.node['имя узла'].sections[‘имя секции’].mount(component options)
